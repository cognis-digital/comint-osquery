...
def to_console(r: ScanResult) -> str:
    ...
    for f in r.findings[:100]:
        ...
        if f.nist_800_53:
            lines.append(f"      📋 NIST 800-53: {f.nist_800_53}")
        if f.disa_stig:
            lines.append(f"      🛡  STIG: {f.disa_stig}")
        if f.mitre_attack:
            lines.append(f"      🎯 ATT&CK: {f.mitre_attack}")
        if f.remediation:
...
def to_markdown(r: ScanResult) -> str:
    ...
    for f in r.findings:
        out.append(
            f"| {f.severity.value} | `{f.id}` | {f.title} | {f.nist_800_53} | {f.disa_stig} | {f.mitre_attack} |"
        )
    out.append("")
    out.append(f"> **{r.classification_placeholder}**")
    return "\n".join(out)


def to_sarif(r: ScanResult) -> str:
    sev_map = {
        Severity.VERY_HIGH: "error",
        Severity.HIGH: "error",
        Severity.MODERATE: "warning",
        Severity.LOW: "note",
        Severity.VERY_LOW: "note",
    }
    return json.dumps(
        {
            "version": "2.1.0",
                            ...
                            "locations": [
                                {
                                    "physicalLocation": {
                                        "artifactLocation": {
                                            "uri": f.location.split(":")[0] if f.location else "unknown"
                                        }
                                    }
                                }
                            ],
    ...
    return json.dumps(
        {
            "assessment-results": {
                ...
                "metadata": {
                    "title": f"{r.tool_name} assessment",
                        ...
                        "findings": [
                            {
                                ...
                                "related-controls": (
                                    [{"control-id": f.nist_800_53}] if f.nist_800_53 else []
                                ),
                            }
