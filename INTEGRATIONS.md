...
**comint-osquery** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `comint-osquery` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
comint-osquery ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
comint-osquery ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
comint-osquery ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `comint-osquery` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="comint-osquery") for rec in records]   # records = your JSON output
